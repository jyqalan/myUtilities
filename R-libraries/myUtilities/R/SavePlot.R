SavePlot <-
function(filename="",path="",size=1,width =(21/2.54)*0.8,height = (29.7/2.54)*0.8,pointsize=12,units='in',res=300,type="jpg") {
  ...
  if(is.na(Type)) {
    ...
    png(filename=filename,width=width,height=height*size,units=units,pointsize=pointsize,bg="white",res=res,restoreConsole=TRUE)
    replayPlot(this.image)
    dev.off()
  } else if(type=="jpg") {
    filename <- paste(make.filename(filename,path),".jpg",sep="")
    jpeg(filename=filename,width=width,height=height*size,units=units,quality=100,pointsize=pointsize,bg="white",res=res,restoreConsole=TRUE)
    replayPlot(this.image)
    dev.off()
  } else if(type=="pdf") {
